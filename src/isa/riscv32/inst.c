...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s -> pc + 4; s -> dnpc = (src1 + imm) & ~1;
                                                                ...
                                                                else if(rd == 0 && R(1) == src1){
                                                                  display_ret(s->pc);
                                                                }));
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = imm = BITS(imm, 4, 0); R(rd) = (SEXT(BITS(src1, 31, 31), 1) << (32 - imm)) | (src1 >> imm)); 
  ...
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulh   , R, uint32_t a = src1; uint32_t b = src2; uint64_t tmp = (uint64_t)a * (uint64_t)b; R(rd) = BITS(tmp, 63, 32));
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(rd)= src1 << imm);
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, tmp = CSR(imm); CSR(imm) = src1; R(rd) = tmp);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, tmp = CSR(imm); CSR(imm) = src1 | tmp; R(rd) = tmp);
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , I, s->dnpc = ECALL(s->pc);etrace());
